...
import { clearCurrentSessionState, getCurrentSessionState, saveCurrentSessionState, saveFavoriteSignature } from './services/storage'
import './App.css'
...
function App() {
  ...
  useEffect(() => {
    void (async () => {
      ...
      const restoredFile = new File([currentSession.document.data], currentSession.document.name, {
        type: currentSession.document.type,
        lastModified: currentSession.document.lastModified,
      })

      setFile(restoredFile)
      setFileUrl(URL.createObjectURL(restoredFile))
      setSignatures(currentSession.signatures)
      setActivePage(currentSession.activePage)
      setScale(currentSession.scale)
      setRotation(currentSession.rotation)
      setStatus('Document restauré hors ligne')
    })()
  }, [])
  ...
  }, [file, fileUrl, signatures, activePage, scale, rotation])
...
function SignatureModal({ onClose, onSave }: { onClose: () => void; onSave: (dataUrl: string, favorite: boolean) => void }) {
  ...
  const [handwritingStyle, setHandwritingStyle] = useState(false)
