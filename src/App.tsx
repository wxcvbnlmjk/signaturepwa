...
function SignatureModal({ onClose, onSave }: { onClose: () => void; onSave: (dataUrl: string, favorite: boolean) => void }) {
  const canvasRef = useRef<SignatureCanvas>(null); const [hasInk, setHasInk] = useState(false)
  const [favorite, setFavorite] = useState(false)
  return <div className="modal-backdrop" role="presentation" onMouseDown={(event) => event.target === event.currentTarget && onClose()}><section className="signature-modal" role="dialog" aria-modal="true" aria-labelledby="signature-title"><div className="modal-header"><div><span className="eyebrow">Votre signature</span><h2 id="signature-title">Dessinez dans le cadre</h2></div><button className="icon-button" onClick={onClose} title="Fermer"><X size={18} /></button></div><div className="ink-board" onPointerDown={() => setHasInk(true)}><SignatureCanvas ref={canvasRef} canvasProps={{ className: 'signature-canvas' }} /></div><div className="modal-hint">Utilisez votre doigt, votre stylet ou votre souris</div><label className="favorite-option"><input type="checkbox" checked={favorite} onChange={(event) => setFavorite(event.target.checked)} />Mémoriser cette signature sur cet appareil</label><div className="modal-actions"><button className="ghost-button" onClick={() => { canvasRef.current?.clear(); setHasInk(false) }}><Eraser size={16} />Effacer</button><button className="primary-button" disabled={!hasInk} onClick={() => onSave(canvasRef.current?.toDataURL('image/png') || '', favorite)}><PenLine size={16} />Utiliser cette signature</button></div></section></div>
}
